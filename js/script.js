// Consegna:
// Scrivi un programma che stampi i numeri da 1 a 100, ma per i multipli di 3 stampi anche "Fizz" di fianco al numero e per i multipli di 5 stampi "Buzz". Per i numeri che sono sia multipli di 3 che di 5 stampi "FizzBuzz".


// Scrivi un programma che stampi i numeri da 1 a 100

for (var i = 1; i <= 100; i++) {
    document.getElementById("lista_numeri").innerHTML += i + "  ";
    
    if (i % 3 == 0 && i % 5 != 0) {
        document.getElementById("lista_numeri").innerHTML += "Fizz ";
    }else if (i % 5 == 0 && i % 3 != 0) {
        document.getElementById("lista_numeri").innerHTML += "Buzz ";
    }else if (i % 15 == 0) {
        document.getElementById("lista_numeri").innerHTML += "Buzz e Fizz ";
    }
}



